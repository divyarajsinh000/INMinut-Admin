...
const AddNews = () => {
  ...
  const handleFileChange = (e) => {
    ...
    // If selectedFiles only contains preloaded default image, clear it so user's custom media replaces it
    setSelectedFiles(prev => {
      const isOnlyDefault = prev.length === 1 && (prev[0].isDefaultImage || prev[0].name === "default_news_image.jpg" || prev[0].name?.startsWith("default_news_image"));
      return isOnlyDefault ? [] : prev;
    });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (notification?.skipped) {
        toast.success("News added. Firebase is not configured, so notification was skipped.");
      } else if (notification) {
        toast.success(`News added. Notifications sent: ${notification.sent || 0}`);
      } else {
        toast.success("News added successfully");
      }
      navigate("/news");
    } catch (error) {
      toast.error(formatErrorMessage(error, "Failed to add news"));
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="flex-1 bg-white rounded-2xl shadow-sm border p-6 w-full lg:max-w-3xl">
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div className="flex items-center justify-between rounded-2xl border border-red-100 bg-red-50/50 p-4">
                <div className="pr-4">
                  <p className="font-bold text-slate-800">Send Notification For This News</p>
                  <p className="text-xs text-slate-500">
                    Turned off by default. Turn on to publish this news and send a push notification.
                  </p>
                </div>
              ...
              <div className="mb-[10px] overflow-hidden rounded-[24px] border border-[#E2E8F0] bg-white shadow-[0_12px_28px_rgba(14,165,233,0.14)]">
                {previewImageUrl && (
                  <div className="relative w-full overflow-hidden bg-white" style={{ height: 300 }}>
                    ...
                    {selectedFiles.filter((file) => getMediaType(file) === 'image').length > 1 && (
                      <div className="absolute bottom-[14px] left-0 right-0 flex justify-center gap-[6px]">
                        ...
                        {Array.from({ length: Math.min(4, selectedFiles.filter((file) => getMediaType(file) === 'image').length - 1) }).map((_, index) => (
                ...
                <div className="p-4">
                  ...
                  <div className="flex items-center justify-between gap-[10px]">
                    ...
                    <div className="flex min-w-0 flex-1 items-center gap-2 pr-2">
                      {user?.profileImage && <img src={getFullMediaUrl(user.profileImage)} alt="Reporter" className="h-[34px] w-[34px] shrink-0 rounded-full object-cover" />}
                      {user?.name && <span className="truncate text-[13px] font-extrabold text-slate-900">{user.name}</span>}
                    </div>
                  )}
